...
def test_get_data_integration(glassnode) -> None:
    ...
    assert list(df.columns) == ['close', 'add_act', 'tx_count'], "Fields are missing from dataframe."
    assert df.index[0][0] == pd.Timestamp('2009-01-12'), "Wrong start date."
    ...
    assert isinstance(df.tx_count.dropna().iloc[-1], np.int64), "Transactions count is not a numpy int."
