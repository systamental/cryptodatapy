...
cc = CryptoCompare()
raw_data = cc.get_data(DataRequest(tickers=['BTC', 'ETH', 'ADA'], fields=['close', 'add_act', 'tx_count']))
raw_ohlc = cc.get_data(DataRequest(tickers=['BTC', 'ETH', 'ADA'], fields=['open', 'high', 'low', 'close', 'volume']))
...
def test_filter_atr(ohlc_df) -> None:
    ...
    assert isinstance(filt_df.index, pd.MultiIndex), "Dataframe should be multiIndex."  # multiindex
    assert isinstance(filt_df.index.droplevel(1), pd.DatetimeIndex), "Index is not DatetimeIndex."  # datetimeindex
    assert all(filt_df.loc[:, :'close'].notna().sum() / ohlc_df.loc[:, :'close'].notna().sum() > 0.95), \
    ...
    assert not any((filt_df.describe().loc['max'] == np.inf) &
                   (filt_df.describe().loc['min'] == -np.inf)), "Inf values found in the dataframe"  # inf
    assert isinstance(filt_df.close.dropna().iloc[-1], np.float64), "Filtered close is not a numpy float."  # dtypes
...
def test_filter_iqr(raw_df) -> None:
    ...
    assert isinstance(filt_df.index, pd.MultiIndex), "Dataframe should be multiIndex."  # multiindex
    assert isinstance(filt_df.index.droplevel(1), pd.DatetimeIndex), "Index is not DatetimeIndex."  # datetimeindex
    assert all(filt_df.notna().sum() / raw_df.notna().sum() > 0.90), \
        "Some series have more than 10% of values filtered as outliers in dataframe."  # % filtered
    assert not any((filt_df.describe().loc['max'] == np.inf) &
                   (filt_df.describe().loc['min'] == -np.inf)), "Inf values found in the dataframe"  # inf
    assert isinstance(filt_df.close.dropna().iloc[-1], np.float64), "Filtered close is not a numpy float."  # dtypes
    assert isinstance(filt_df.add_act.dropna().iloc[-1], np.int64), \
        "Filtered active addresses are not a numpy int."  # dtypes
    assert isinstance(filt_df.tx_count.dropna().iloc[-1], np.int64), \
...
def test_filter_mad(raw_df) -> None:
    ...
    assert isinstance(filt_df.index, pd.MultiIndex), "Dataframe should be multiIndex."  # multiindex
    assert isinstance(filt_df.index.droplevel(1), pd.DatetimeIndex), "Index is not DatetimeIndex."  # datetimeindex
    assert all(filt_df.notna().sum() / raw_df.notna().sum() > 0.80), \
...
def test_filter_zscore(raw_df) -> None:
    ...
    assert isinstance(filt_df.index, pd.MultiIndex), "Dataframe should be multiIndex."  # multiindex
    assert isinstance(filt_df.index.droplevel(1), pd.DatetimeIndex), "Index is not DatetimeIndex."  # datetimeindex
    assert all(filt_df.notna().sum() / raw_df.notna().sum() > 0.9), \
        "Some series have more than 10% of values filtered as outliers in dataframe."  # % filtered
    assert not any((filt_df.describe().loc['max'] == np.inf) &
                   (filt_df.describe().loc['min'] == -np.inf)), "Inf values found in the dataframe"  # inf
    assert isinstance(filt_df.close.dropna().iloc[-1], np.float64), "Filtered close is not a numpy float."  # dtypes
...
def test_filter_seasonal_decomp(raw_df) -> None:
    ...
    assert isinstance(filt_df.index, pd.MultiIndex), "Dataframe should be multiIndex."  # multiindex
    assert isinstance(filt_df.index.droplevel(1), pd.DatetimeIndex), "Index is not DatetimeIndex."  # datetimeindex
    assert all(filt_df.notna().sum() / raw_df.notna().sum() > 0.7), \
...
def test_filter_prophet(raw_df) -> None:
    ...
    assert isinstance(filt_df.index, pd.MultiIndex), "Dataframe should be multiIndex."  # multiindex
    assert isinstance(filt_df.index.droplevel(1), pd.DatetimeIndex), "Index is not DatetimeIndex."  # datetimeindex
    assert all(filt_df.notna().sum() / raw_df.notna().sum() > 0.8), \
        "Some series have more than 20% of values filtered as outliers in dataframe."  # % filtered
    assert not any((filt_df.describe().loc['max'] == np.inf) &
                   (filt_df.describe().loc['min'] == -np.inf)), "Inf values found in the dataframe"  # inf
    assert isinstance(filt_df.close.dropna().iloc[-1], np.float64), "Filtered close is not a numpy float."  # dtypes
...
def test_filter_avg_trading_vals(ohlc_df) -> None:
    ...
    assert isinstance(filt_df.index, pd.MultiIndex), "Dataframe should be multiIndex."  # multiindex
    assert isinstance(filt_df.index.droplevel(1), pd.DatetimeIndex), "Index is not DatetimeIndex."  # datetimeindex
    assert filt_df.loc[pd.IndexSlice[:'2011-01-01', 'BTC'], 'close'].isnull().sum() > 0, \
...
def test_filter_missing_vals_gaps(ohlc_df) -> None:
    ...
    assert isinstance(filt_df.index, pd.MultiIndex), "Dataframe should be multiIndex."  # multiindex
    assert isinstance(filt_df.index.droplevel(1), pd.DatetimeIndex), "Index is not DatetimeIndex."  # datetimeindex
    assert filt_df.loc[pd.IndexSlice[:, 'BTC'], 'close'].dropna().index[0][0] == pd.Timestamp('2014-12-09 00:00:00'),\
...
def test_filter_min_nobs(ohlc_df) -> None:
    ...
    assert isinstance(filt_df.index, pd.MultiIndex), "Dataframe should be multiIndex."  # multiindex
    assert isinstance(filt_df.index.droplevel(1), pd.DatetimeIndex), "Index is not DatetimeIndex."  # datetimeindex
    assert 'BTC' not in list(filt_df.index.droplevel(0).unique()), "BTC should be removed from dataframe"  # filt vals
    assert not any((filt_df.describe().loc['max'] == np.inf) &
                   (filt_df.describe().loc['min'] == -np.inf)), "Inf values found in the dataframe"  # inf
    assert isinstance(filt_df.close.dropna().iloc[-1], np.float64), "Filtered close is not a numpy float."  # dtypes
