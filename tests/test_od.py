...
def test_od_atr(ohlc_df) -> None:
    ...
    # assert statements
    assert outliers_df.shape == ohlc_df.shape, "Outliers dataframe changed shape."  # shape
    assert isinstance(outliers_df.index, pd.MultiIndex), "Dataframe should be multiIndex."  # multiindex
    assert isinstance(outliers_df.index.droplevel(1), pd.DatetimeIndex), "Index is not DatetimeIndex."  # datetimeindex
    assert all(outliers_df.loc[:, :'close'].notna().sum() / ohlc_df.loc[:, :'close'].notna().sum() < 0.05), \
        "Some series have more than 5% of values filtered as outliers in dataframe."  # % outliers
    assert not any((outliers_df.describe().loc['max'] == np.inf) &
                   (outliers_df.describe().loc['min'] == -np.inf)), "Inf values found in the dataframe"  # inf
...
def test_od_iqr(raw_df) -> None:
    ...
    outliers_df = df['outliers']
    # assert statements
    assert outliers_df.shape == raw_df.shape, "Outliers dataframe changed shape."  # shape
    assert isinstance(outliers_df.index, pd.MultiIndex), "Dataframe should be multiIndex."  # multiindex
    assert isinstance(outliers_df.index.droplevel(1), pd.DatetimeIndex), "Index is not DatetimeIndex."  # datetimeindex
    assert all(outliers_df.notna().sum() / raw_df.notna().sum() < 0.05), \
...
def test_od_mad(raw_df) -> None:
    ...
    assert isinstance(outliers_df.index, pd.MultiIndex), "Dataframe should be multiIndex."  # multiindex
    assert isinstance(outliers_df.index.droplevel(1), pd.DatetimeIndex), "Index is not DatetimeIndex."  # datetimeindex
    assert all(outliers_df.notna().sum() / raw_df.notna().sum() < 0.2), \
...
def test_od_stl(raw_df) -> None:
    ...
    assert isinstance(outliers_df.index, pd.MultiIndex), "Dataframe should be multiIndex."  # multiindex
    assert isinstance(outliers_df.index.droplevel(1), pd.DatetimeIndex), "Index is not DatetimeIndex."  # datetimeindex
    assert all(outliers_df.notna().sum() / raw_df.notna().sum() < 0.25), \
...
def test_od_prophet(raw_df) -> None:
    ...
    assert isinstance(outliers_df.index, pd.MultiIndex), "Dataframe should be multiIndex."  # multiindex
    assert isinstance(outliers_df.index.droplevel(1), pd.DatetimeIndex), "Index is not DatetimeIndex."  # datetimeindex
    assert all(outliers_df.notna().sum() / raw_df.notna().sum() < 0.1), \
