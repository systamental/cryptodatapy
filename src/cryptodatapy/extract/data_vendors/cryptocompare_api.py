...
class CryptoCompare(DataVendor):
    ...
    def get_exchanges_info(
        ...
        while attempts < 3:
            ...
            try:  # try get request
                ...
                sleep(0.1)
                logging.warning(
                    f"Failed to get exchanges info after attempt #{str(attempts)}."
                )
                if attempts == 3:
    ...
    def get_indexes_info(self, as_list: bool = False) -> Union[List[str], pd.DataFrame]:
        ...
        while attempts < 3:
            ...
            try:  # try get request
                ...
                sleep(0.1)
                logging.warning(
                    f"Failed to get indexes info after attempt #{str(attempts)}."
                )
                if attempts == 3:
    ...
    def get_assets_info(self, as_list: bool = False) -> Union[List[str], pd.DataFrame]:
        ...
        while attempts < 3:
            ...
            try:  # try get request
                ...
                sleep(0.1)
                logging.warning(
                    f"Failed to get asset info after attempt #{str(attempts)}."
                )
                if attempts == 3:
    ...
    def get_top_market_cap_info(self, n: int = 100) -> List[str]:
        ...
        if n > 100:
            raise ValueError(
                "Maximum number of assets is 100. Change n parameter and try again."
            )
        ...
        while attempts < 3:
            ...
            try:  # try get request
                ...
                params = {"limit": n, "tsym": "USD", "api_key": self.api_key}
                r = requests.get(url, params=params)
                ...
                sleep(0.1)
                logging.warning(
                    f"Failed to get top market cap assets after attempt #{str(attempts)}."
                )
                if attempts == 3:
                    logging.warning(
                        "Failed to get top market cap assets after many attempts."
                    )
                    break
    ...
    def get_markets_info(
        ...
        while attempts < 3:
            ...
            try:  # try get request
                ...
                sleep(0.1)
                logging.warning(
                    f"Failed to get markets info after attempt #{str(attempts)}."
                )
                if attempts == 3:
    ...
    def get_onchain_info(self, as_list: bool = False) -> Union[List[str], pd.DataFrame]:
        ...
        while attempts < 3:
            ...
            try:  # try get request
                ...
                sleep(0.1)
                logging.warning(
                    f"Failed to get on-chain info after attempt #{str(attempts)}."
                )
                if attempts == 3:
                    logging.warning(
                ...
                # format date
                onchain["data_available_from"] = pd.to_datetime(
                    onchain.data_available_from, unit="s"
                )
                # add index name
    ...
    def get_fields_info(self, data_type: Optional[str] = None) -> List[str]:
        ...
        """
        ohlcv_list, onchain_list, social_list = (
        ...
        while attempts < 3:
            ...
            try:  # try get request for social stats
                ...
                sleep(0.1)
                logging.warning(
                    f"Failed to get social stats info after attempt #{str(attempts)}."
                )
                if attempts == 3:
                    logging.warning(
    ...
    def get_rate_limit_info(self) -> pd.DataFrame:
        ...
        while attempts < 3:
            ...
            try:  # try get request
                ...
                sleep(0.1)
                logging.warning(
                    f"Failed to get rate limit info after attempt #{str(attempts)}."
                )
                if attempts == 3:
                    logging.warning(
    ...
    def get_indexes(self, data_req: DataRequest) -> pd.DataFrame:
        ...
        cc_data_req = ConvertParams(
            data_req, data_source="cryptocompare"
        ).convert_to_source()
        ...
        if len(tickers) == 0:
            raise ValueError(
                f"Tickers are all assets. Available indexes include: {self.indexes}."
            )
        ...
        for ticker in tickers:
            ...
            while attempts < cc_data_req["trials"]:
                try:  # fetch index data
                    ...
                    if (
                        len(data) < (self.max_obs_per_call + 1)
                        or data.close[0] == 0
                        or data.close[0].astype(str) == "nan"
                    ):
                        break
    ...
    def get_ohlcv(self, data_req: DataRequest) -> pd.DataFrame:
        ...
        if len(tickers) == 0:
            raise ValueError(
                f"Tickers are all indexes. Use assets property to see available assets."
            )
    ...
    def get_onchain(self, data_req: DataRequest) -> pd.DataFrame:
        ...
        if cc_data_req["freq"] != "histoday":
            raise ValueError(
                f"On-chain data is only available on a daily frequency."
        ...
        if len(tickers) == 0:
            raise ValueError(
                "Tickers are all indexes. Use assets property to see available assets."
            )
        ...
        for ticker in tickers:
            ...
            while attempts < cc_data_req["trials"]:
                try:
                    ...
                    if (
                        len(data) < (self.max_obs_per_call - 1)
                        or all(data.iloc[0] == 0)
                        or all(data.iloc[0].astype(str) == "nan")
                    ):
                        break
                    # reset end date and pause before calling API
                    else:
    ...
    def get_social(self, data_req: DataRequest) -> pd.DataFrame:
        ...
        if cc_data_req["freq"] != "histoday" and cc_data_req["freq"] != "histohour":
            raise ValueError(
                f"Social stats data is only available on a daily and hourly frequency."
        ...
        if len(tickers) == 0:
            raise ValueError(
                "Tickers are all indexes. Use assets property to see all available assets."
            )
        ...
        for ticker in tickers:
            ...
            while attempts < cc_data_req["trials"]:
                try:
                    ...
                    if (
                        len(data) < (self.max_obs_per_call + 1)
                        or all(data.drop(columns=["time"]).iloc[0] == 0)
                        or all(data.drop(columns=["time"]).iloc[0].astype(str) == "nan")
                    ):
                        break
                    # reset end date and pause before calling API
                    else:
    ...
    def get_data(self, data_req: DataRequest) -> pd.DataFrame:
        ...
        if not all(i in fields_list for i in cc_data_req["fields"]):
            raise ValueError(
                "Fields are not available. Use fields property to see all available fields."
            )
        ...
        # fetch indexes data
        if any(i in idx_tickers_list for i in cc_data_req["tickers"]) and any(
            i in ohlcv_list for i in cc_data_req["fields"]
        ):
            try:
