...
def test_get_assets_info(investpy) -> None:
    ...
    ip = investpy
    assert (
        ip.get_assets_info()["rates"].loc["U.S. 10Y", "country"] == "united states"
...
def test_get_indexes_info(investpy) -> None:
    ...
    ip = investpy
    assert (
        ip.get_indexes_info().loc["SPX", "name"] == "S&P 500"
    ), "Index info is missing 'SPX'."
    assert (
        "BCOM" in ip.get_indexes_info(as_dict=True)["cmdty"]
    ), "Indexes are missing 'BCOM'."
...
def test_fields(investpy) -> None:
    ...
    """
    ip = investpy
    assert ip.fields["macro"] == [
        "actual",
        "previous",
        "expected",
        "surprise",
    ], "Fields for macro cat are incorrect."
...
def test_get_indexes(investpy) -> None:
    ...
    data_req = DataRequest(
        tickers=["US_Eqty_Idx", "JP_Eqty_Idx", "EZ_Eqty_Idx"], cat="eqty"
    )
...
def test_get_cmdty(investpy) -> None:
    ...
    ip = investpy
    data_req = DataRequest(
        tickers=["Gold", "Crude_Oil_Brent", "Copper", "Uranium"], cat="cmdty"
    )
